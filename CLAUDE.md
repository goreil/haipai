...
**Frontend — features**
- Concept-level EV breakdown (two ledgers at the top of a game — "Losing points here" = pills the AI won / under-used concepts; "Overvaluing these" = pills you won on a losing play / over-prioritized concepts): aggregator `static/js/game-concept-breakdown.js` (re-buckets the `compare-dimensions.js` win-vector both per side×dim AND per side×group, dedups deal_in's per-seat vector + the per-group double-count, attributes full `ev_loss` to every concept a mistake touches), rendered by `renderConceptBreakdown` + `renderTopGroupStat` in `static/js/game-render.js` (breakdown under the summary bar; the single deduped top group also shows as a "Top leak" stat IN the summary bar). Group colour scheme (Efficiency=blue, Yaku=green, Value=gold, Defense=pink) is the single shared source `haipaiCompareDimensions.GROUP_META` (`compare-dimensions.js`), consumed by the EV-table feature pills (`.feat-pill-grp` via `--feat-grp` in `ev-table.js`) AND the breakdown's first-column pills (`.concept-pill` via `--grp`). Styles `.concept-breakdown` / `.feat-pill-grp` in `static/style-game-detail.css`
