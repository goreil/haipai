...
- Leaderboard nickname (the Account page's "Display name" — **the minigame boards only**; login, uniqueness and every admin view still key on `username`): nullable `users.display_name` (`db/schema.py`, with a case-insensitive unique index on `lower(display_name)`), `validate_display_name`/`set_display_name` in `db/users.py` (the latter also refuses any *username*, so a nickname can't impersonate an account; setting your own username stores NULL). Empty clears it — the boards read `COALESCE(display_name, username)` in `db/waits.py` + `db/defense.py`, so **NULL is the default and nothing needs backfilling**. Endpoint `POST /api/me/display-name` + the `display_name` field on `/api/me` and the GDPR export (`routes/auth.py`); UI + `saveDisplayName` in `static/js/account.js` (action in `static/js/actions.js`), styles `.account-input`/`.account-field`/`.account-msg` (`static/style-layout.css`). Pinned by `tests/test_api_display_name.py`.
...
- Minigames — Waits Trainer (`#waits-trainer`), Defense Trainer (`#defense-trainer`),
  Efficiency Trainer (`#efficiency-trainer`), and the public guest arcade at `/play`:
  self-contained client-side games sharing `minigame-audio.js` / `minigame-shell.js`,
  with server-side leaderboards only. **`MG_GAMES` in `static/js/minigame-shell.js` is
  the roster** — the single list of which minigames exist. Both hash routers
