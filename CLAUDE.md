...
- Chrome MV3 auto-uploader (mjai report page → `POST /api/games/upload` → `#g<id>`): `extension/` (`manifest.json`, `background.js` = service worker doing the cookie-authenticated cross-origin POST + dedupe/retry/navigate, `content.js` = report-page `?data=` guard + fetch + toast + log-in prompt with session polling, `options.html`/`options.js`, `README.md`). Vanilla JS, no build step; not served by `static/`, not in the Docker image. Contract + hard constraints (never POST to mjai's `/review`, never expose the session to page context): `docs/backlogs/Browser-extension-spec.md`. Server side of the flow is `api_upload` in `routes/game.py`; the older bookmarklet it coexists with is built in `static/js/ui.js`. Auth is the session cookie (see "Extension auth" above), NOT a pasted upload token.
