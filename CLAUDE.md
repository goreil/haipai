...
- Extension login (how the browser extension authenticates — replaces pasting an upload token; the bookmarklet's `users.upload_token` is untouched and still works): the extension opens `GET|POST /extension/authorize` (`routes/auth.py`) via `chrome.identity.launchWebAuthFlow`, the user logs in normally (password/Discord/Google — which is *why* it's a real login page and not a form in the extension: OAuth accounts have no password), approves `templates/extension_authorize.html`, and the server mints a row in the `extension_tokens` table (`db/schema.py`; CRUD in `db/users.py` — `create_extension_token`/`get_user_by_extension_token`/`list_extension_tokens`/`delete_extension_token`/`delete_extension_token_by_value`). `api_upload` accepts either credential. Management: `GET`/`DELETE /api/me/extension-tokens[/<id>]` + the Account-page panel (`renderExtensionTokens`/`revokeExtensionToken` in `static/js/account.js`), and extension-side self-revoke `POST /api/extension/revoke` (CSRF-exempt in `app.py`, Bearer-authed like `api_upload`). **Do not "simplify" this to session-cookie auth**: Chrome does attach the `SameSite=Lax` cookie to extension-worker fetches with host permissions (measured), but `WTF_CSRF_SSL_STRICT` then rejects the POST for a missing `Referer` that an extension cannot send — making it work would require CSRF-exempting a cookie-authenticated endpoint. Rationale + the measurements: `docs/backlogs/Browser-extension-spec.md` "Authentication". Landing back on the consent page after login is `stash_post_login`/`_post_login_target` in `routes/auth.py` (session-stashed, so it survives the OAuth round-trip). Tests: `tests/test_api_extension.py`.
- Email verification (password-based registration only — Discord/Google logins have no `email` on file and skip this entirely): `users.email`/`email_verified`/`email_verify_token`/`email_verify_expires` (`db/schema.py`; existing pre-feature accounts were backfilled `email_verified=1` in the same migration, so the gate only ever applies going forward). CRUD in `db/users.py` (`create_user` now takes optional `email`/`verify_token`/`verify_expires`; `get_user_by_email`, `get_user_by_verify_token`, `mark_email_verified`, `set_verify_token`). Registration (`routes/auth.py` `register()`) requires an email, creates the account unverified, and does NOT log the user in — it sends a 24h-expiry link via `lib/mail.send_verification_email` (plain `smtplib` over mailbox.org SMTP, `MAILBOX_USERNAME`/`MAILBOX_PASSWORD`/optional `MAIL_FROM` env vars — must be listed in `docker-compose.yml`'s `environment:` block, not just `.env`, or the container never sees them). `login()` hard-blocks password accounts with an unverified `email` and renders a "Resend verification email" button (`unverified_username` in `templates/login.html`) posting to `/resend-verification`; `GET /verify-email/<token>` marks verified and logs the user in.
...
- Chrome MV3 auto-uploader (mjai report page → `POST /api/games/upload` → `#g<id>`): `extension/` (`manifest.json`, `background.js` = service worker owning the credential + sign-in + the cross-origin POST, `content.js` = report-page `?data=` guard + fetch + toast + sign-in prompt, `options.html`/`options.js`, `README.md`). Vanilla JS, no build step; not served by `static/`, not in the Docker image. Contract + hard constraints (never POST to mjai's `/review`, never expose the credential to page context): `docs/backlogs/Browser-extension-spec.md`. Server side of the flow is `api_upload` in `routes/game.py`; the older bookmarklet it coexists with is built in `static/js/ui.js`. Auth is the extension-login flow below, NOT the pasted upload token.
