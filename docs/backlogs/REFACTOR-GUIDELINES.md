...
## Principles (enforce on every refactor PR)
...
1. **Inventory before action, target shape before code.** Write down
   ...
   PR moves toward it. No ad-hoc reshaping.
2. **One axis per PR.** Axes are: (a) collapse duplications, (b) split
   oversized files, (c) rename for findability, (d) unify data shapes,
   (e) kill legacy formats. Mixing axes in one PR makes regressions
   ambiguous.
...
- **Schema is a wall.** `games.db` is live production. Refactors are
  ...
  sneak schema changes into reshape PRs.
