...
- Textual justifications we can surface as trainer tips in the review UI and practice mode.
...
- [ ] Build a `lib/tips.py` (or extend `categorize.py`) that attaches a short tip + book reference to each categorized mistake, surfaced in the review view and practice mode.
