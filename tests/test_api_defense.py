#!/usr/bin/env python3
"""Tests for the Defense Trainer minigame API (/api/defense/*).

Covers the self-reported-score gate in `routes/defense.py`, the shape of the
leaderboard, and that the board is per-player-best (not per-run). Sibling of
tests/test_api_waits.py — the two APIs are deliberately the same shape.
"""
...
def _submit(client, score, best_streak, steps_cleared):
    return client.post("/api/defense/scores", json={
        "score": score,
...
class TestSubmitScore:
    def test_requires_login(self, client):
        ...
        assert client.get("/api/defense/leaderboard").status_code == 401

    def test_records_a_run(self, client):
        _login(client)
        res = _submit(client, 40, 6, 8)
        assert res.status_code == 200
        body = res.get_json()
        assert body["recorded"] is True
        assert body["leaderboard"]["top"][0]["score"] == 40
    ...
    def test_rejects_score_above_the_whole_arsenal_per_step(self, client):
        # A step is worth its safe-tile count, and 34 tiles is the ceiling.
        _login(client)
        assert _submit(client, 35, 1, 1).status_code == 400
        assert _submit(client, 34, 1, 1).status_code == 200

    def test_rejects_score_below_one_point_per_step(self, client):
        # Every step has at least one safe tile — the riichi declaration.
        _login(client)
        assert _submit(client, 3, 1, 4).status_code == 400

    def test_rejects_streak_above_steps_cleared(self, client):
        _login(client)
        assert _submit(client, 30, 9, 8).status_code == 400
